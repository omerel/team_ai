...
Multi-writer page creation IS safe in v1.7 because every page write is gated by `.claude/scripts/wiki-lock.sh`. Acquire before write, release after:

```bash
bash .claude/scripts/wiki-lock.sh acquire wiki/sources/<slug>.md || {
...
The lock semantics (age-based, 60s default stale window, cross-process release allowed) are documented in `.claude/scripts/wiki-lock.sh` and `skills/wiki-ingest/SKILL.md` §Concurrency. There is no opt-out; this is core in v1.7.
...
- Write any wiki/ file WITHOUT first acquiring its lock via `.claude/scripts/wiki-lock.sh acquire` (v1.7+ concurrency rule above)
