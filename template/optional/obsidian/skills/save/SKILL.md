...
The session-note write itself follows the standard transport policy. Read `.vault-meta/transport.json` (auto-created by `bash .claude/scripts/detect-transport.sh`):

- **mcp-obsidian** / **mcpvault** — `mcp__obsidian-vault__write_note`
- **filesystem** — Claude's `Write` tool with absolute path

Index/log/hot updates use the same transport.
...
When filing into the project's own `wiki/`, use the canonical folders from the Note Type table below. When filing into a personal-vault root, use the canonical folders documented in that vault's CLAUDE.md (commonly `sessions/`, `concepts/`, `sources/`). Filename sanitization (slug + safe_name) still applies regardless of root: strip path separators, NUL bytes, control chars, leading dots/hyphens.
