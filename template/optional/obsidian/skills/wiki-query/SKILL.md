...
Reads should prefer the same transport the rest of the plugin uses. Consult `.vault-meta/transport.json` (auto-created by `bash .claude/scripts/detect-transport.sh`) and use the `preferred` entry:
...
Quick mode (hot.md only) is transport-agnostic — always uses `Read`.
...
| 6 | CONNECT (sys) | Hot cache + index + page drill-down layer into a single read pipeline. |
...
| 10 | GROW | Questions the wiki can't answer are content gaps — log them and suggest finding a source. |
