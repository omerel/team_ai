...
Lint primarily reads, then writes a single report file. Both follow the standard transport policy. Read `.vault-meta/transport.json` (auto-created by `bash .claude/scripts/detect-transport.sh`):

- **mcp-obsidian** / **mcpvault** — `mcp__obsidian-vault__read_multiple_notes`, `list_all_tags`
- **filesystem** — Claude's `Read`/`Glob`/`Grep` (final floor; always works)
...
| 6 | CONNECT (sys) | Lint checks + Dataview dashboards + canvas overview — multiple lint surfaces to integrate. |
